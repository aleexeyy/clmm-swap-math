...
pub fn compute_swap_step(
    ...
    let zero_for_one: bool = sqrt_ratio_current_x96 >= sqrt_ratio_target_x96;
    let exact_in: bool = amount_remaining >= I256::ZERO;
    ...
    if exact_in {
        ...
        sqrt_ratio_next_x96 = if (-amount_remaining).into_raw() >= amount_out {
            sqrt_ratio_target_x96
        } else {
    ...
    let max: bool = sqrt_ratio_target_x96 == sqrt_ratio_next_x96;

    if zero_for_one {
        amount_in = if max && exact_in {
            amount_in
        } else {
            get_amount_0_delta_base(sqrt_ratio_next_x96, sqrt_ratio_current_x96, liquidity, true)?
        };
        ...
        amount_in = if max && exact_in {
            ...
            get_amount_1_delta_base(sqrt_ratio_current_x96, sqrt_ratio_next_x96, liquidity, true)?
        };
